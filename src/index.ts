...
const PORT = process.env.AGENT_PORT
  ? parseInt(process.env.AGENT_PORT, 10)
  : 7723;
...
app.use("/stats", async (c, next) => {
  ...
  if (!token || token !== AGENT_TOKEN) {
    return c.json(
      {
        error: "Unauthorized",
        message: 'A valid "agent_token" header is required.',
      },
      401,
    );
  }
...
const getMemoryUsage = async (): Promise<object> => {
  try {
    ...
    const [total, used, free, shared, buff_cache, available] = stdout
      .trim()
      .split(/\s+/);
    return {
      total: `${total}MB`,
...
app.get("/stats", async (c) => {
  const [mysqlStatus, postgresStatus, mongoStatus, cpuUsage, memoryUsage] =
    await Promise.all([
